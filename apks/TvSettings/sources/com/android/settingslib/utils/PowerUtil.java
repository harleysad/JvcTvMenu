...
public class PowerUtil {
    ...
    private static String getMoreThanOneDayString(Context context, long drainTimeMs, String percentageString, boolean basedOnUsage) {
        ...
        if (TextUtils.isEmpty(percentageString)) {
            ...
            return context.getString(id2, new Object[]{timeString});
        }
        if (basedOnUsage) {
    ...
    private static String getMoreThanTwoDaysString(Context context, String percentageString) {
        MeasureFormat frmt = MeasureFormat.getInstance(context.getResources().getConfiguration().getLocales().get(0), MeasureFormat.FormatWidth.SHORT);
        Measure daysMeasure = new Measure(2, MeasureUnit.DAY);
        if (TextUtils.isEmpty(percentageString)) {
            return context.getString(R.string.power_remaining_only_more_than_subtext, new Object[]{frmt.formatMeasures(new Measure[]{daysMeasure})});
        }
        return context.getString(R.string.power_remaining_more_than_subtext, new Object[]{frmt.formatMeasures(new Measure[]{daysMeasure}), percentageString});
    }
