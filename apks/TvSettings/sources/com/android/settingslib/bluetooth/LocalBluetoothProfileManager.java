...
public class LocalBluetoothProfileManager {
    ...
    LocalBluetoothProfileManager(Context context, LocalBluetoothAdapter adapter, CachedBluetoothDeviceManager deviceManager, BluetoothEventManager eventManager) {
        ...
        addProfile(this.mHidProfile, "HID", BluetoothDevicePairer.ACTION_CONNECTION_STATE_CHANGED);
        this.mPanProfile = new PanProfile(context, this.mLocalAdapter);
        addPanProfile(this.mPanProfile, MtkTvConfigTypeBase.S3166_CFG_COUNT_PAN, "android.bluetooth.pan.profile.action.CONNECTION_STATE_CHANGED");
    ...
    private class StateChangedHandler implements BluetoothEventManager.Handler {
        ...
        public void onReceiveInternal(Intent intent, CachedBluetoothDevice cachedDevice) {
            ...
            boolean isHearingAidProfile = false;
            ...
            if (LocalBluetoothProfileManager.this.getHearingAidProfile() != null && (this.mProfile instanceof HearingAidProfile)) {
                isHearingAidProfile = true;
            }
            if (isHearingAidProfile && newState == 2 && cachedDevice.getHiSyncId() == 0) {
            ...
            if (isHearingAidProfile && (otherDevice = LocalBluetoothProfileManager.this.mDeviceManager.getHearingAidOtherDevice(cachedDevice, cachedDevice.getHiSyncId())) != null) {
                Log.d(LocalBluetoothProfileManager.TAG, "Refreshing other hearing aid=" + otherDevice + ", newState=" + newState);
                otherDevice.refresh();
            }
