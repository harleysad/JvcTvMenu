...
public class CachedBluetoothDeviceManager {
    ...
    public synchronized String getHearingAidPairDeviceSummary(CachedBluetoothDevice device) {
        ...
        CachedBluetoothDevice otherHearingAidDevice = getHearingAidOtherDevice(device, device.getHiSyncId());
        if (otherHearingAidDevice != null) {
            pairDeviceSummary = otherHearingAidDevice.getConnectionSummary();
        }
        log("getHearingAidPairDeviceSummary: pairDeviceSummary=" + pairDeviceSummary + ", otherHearingAidDevice=" + otherHearingAidDevice);
    ...
    public CachedBluetoothDevice getHearingAidOtherDevice(CachedBluetoothDevice thisDevice, long hiSyncId) {
        if (hiSyncId == 0) {
            return null;
        }
        for (CachedBluetoothDevice notCachedDevice : this.mHearingAidDevicesNotAddedInCache) {
