...
public class DataUsageController {
    ...
    public long getDefaultWarningLevel() {
        return PlaybackStateCompat.ACTION_SET_CAPTIONING_ENABLED * ((long) this.mContext.getResources().getInteger(17694947));
    }
        ...
        try {
            ...
            try {
                ...
                if (DEBUG) {
                    try {
                        long j = now;
                        Log.d(TAG, String.format("history call from %s to %s now=%s took %sms: %s", new Object[]{new Date(start2), new Date(end3), new Date(now), Long.valueOf(callEnd - callStart), historyEntryToString(entry)}));
    ...
    private String formatDateRange(long start, long end) {
        synchronized (PERIOD_BUILDER) {
            try {
                ...
                String formatter = DateUtils.formatDateRange(this.mContext, PERIOD_FORMATTER, start, end, 65552, (String) null).toString();
                return formatter;
            } catch (Throwable th) {
                th = th;
                throw th;
