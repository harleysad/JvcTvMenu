...
public class MetricsFeatureProvider {
    ...
    public void action(VisibilityLoggerMixin visibilityLogger, int category, int value) {
        for (LogWriter writer : this.mLoggerWriters) {
            writer.action(category, value, (Pair<Integer, Object>[]) new Pair[]{sinceVisibleTaggedData(visibilityLogger.elapsedTimeSinceVisible())});
        }
    }
        ...
        if (intent != null) {
            ...
            if (cn == null) {
                ...
                if (!TextUtils.isEmpty(action)) {
                    action(context, 830, action, Pair.create(833, Integer.valueOf(sourceMetricsCategory)));
                }
            } else if (!TextUtils.equals(cn.getPackageName(), context.getPackageName())) {
                action(context, 830, cn.flattenToString(), Pair.create(833, Integer.valueOf(sourceMetricsCategory)));
            }
        }
    }

    private Pair<Integer, Object> sinceVisibleTaggedData(long timestamp) {
        return Pair.create(794, Long.valueOf(timestamp));
    }
}
