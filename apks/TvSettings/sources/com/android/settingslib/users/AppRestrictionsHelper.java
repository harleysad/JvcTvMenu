    ...
    public void applyUserAppsStates(OnDisableUiForPackageListener listener) {
        if (this.mRestrictedProfile || this.mUser.getIdentifier() == UserHandle.myUserId()) {
            for (Map.Entry<String, Boolean> entry : this.mSelectedPackages.entrySet()) {
                applyUserAppState(entry.getKey(), entry.getValue().booleanValue(), listener);
            }
            return;
        }
        ...
        if (enabled) {
            ...
            try {
                ...
                if (this.mRestrictedProfile) {
                    this.mPackageManager.deletePackageAsUser(packageName, (IPackageDeleteObserver) null, 4, this.mUser.getIdentifier());
                    return;
                }
                listener.onDisableUiForPackage(packageName);
        ...
        for (ResolveInfo app : pm.queryIntentActivities(intent, 8704)) {
            if (!(app.activityInfo == null || app.activityInfo.applicationInfo == null)) {
                ...
                if (!((flags & 1) == 0 && (flags & 128) == 0) && !excludePackages.contains(packageName)) {
                    ...
                    if (!((enabled == 4 || enabled == 2) && ((targetUserAppInfo = getAppInfoForUser(packageName, 0, this.mUser)) == null || (targetUserAppInfo.flags & 8388608) == 0))) {
                        SelectableAppInfo info = new SelectableAppInfo();
