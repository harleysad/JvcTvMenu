    ...
    public final void startScroll(int dx, int dy, boolean easeFling, int duration, boolean page) {
        ...
        if (this.mScrollMode != 0) {
            ...
            if (easeFling) {
                ...
                int durationY2 = i;
                if (duration == 0) {
                    durationY = Math.max(durationX, durationY2);
                } else {
                    durationY = duration;
                }
                boolean z = page;
            } else if (duration == 0) {
                durationY = getScrollDuration((int) Math.sqrt((double) ((dx2 * dx2) + (dy2 * dy2))), page);
            } else {
                boolean z2 = page;
                durationY = duration;
            }
            scroller.startScroll(basex, basey, dx2, dy2, durationY);
        }
    }
