...
public class ActionAdapter extends BaseAdapter implements ScrollAdapter, ScrollAdapterView.OnScrollListener, View.OnKeyListener, View.OnClickListener {
    ...
    private void fadeIn(View v) {
        ...
        alphaAnimator.setDuration((long) this.mContext.getResources().getInteger(17694721));
        alphaAnimator.start();
    }
        ...
        if (!(i == 66 || i == 160)) {
            switch (i) {
                ...
                case 22:
                    ...
                    if (action.isEnabled() && action.hasLeftRight()) {
                        cando = true;
                    }
                    MtkLog.d(TAG, "press left-right key can change value:" + cando);
                    if (!cando) {
                        return false;
                    }
                    changeValueForLeftRight(i, action, view);
                    return true;
                case 23:
                    break;
                default:
    ...
    private void changeValueForLeftRight(int keycode, Action action, View SelView) {
        ...
        ((TextView) SelView.findViewById(R.id.action_description)).setText(action.getDescription());
        MtkLog.d(TAG, "motor callback ...itemId:" + action.mItemID + ",description:" + action.getDescription());
        if (action.mItemID.equals(MenuConfigManager.DVBS_DETAIL_DISEQC_MOTOR)) {
            ...
            if (action.getDescription().equalsIgnoreCase(DISEQC_12_ITEM)) {
                ...
                notifyDataSetChanged();
                MtkLog.d(TAG, "motor  notifyDataSetChanged...");
            }
        }
